...
# ======================= OPD math (reuses OPD's ttrl_math reward) =======================

def create_opd_math_reward_fn() -> Callable:
    """Reward function for OPD math parquet rows scored via ttrl_math.reward_func.

    Expects rows in the verl parquet format (as used by on_policy_distillation.sh):
        {"data_source": str, "reward_model": {"ground_truth": str, ...}, ...}
    The student response is the raw vLLM text completion.
    """
    from verl.utils.reward_score.ttrl_math import reward_func as ttrl_reward_func

    def reward_fn(response: str, task_data: Dict[str, Any]) -> Dict[str, Any]:
        if "reward_model" in task_data and "ground_truth" in task_data["reward_model"]:
            ground_truth = str(task_data["reward_model"]["ground_truth"])
        else:
            ground_truth = str(task_data.get("answer", task_data.get("solution", "")))

        data_source = task_data.get("data_source", "math_dapo")
        extra_info = task_data.get("extra_info", None)

        try:
            res = ttrl_reward_func(
            ...
            print(f"[opd_math reward_fn] grading failed: {e}")
            res = 0.0

        if isinstance(res, dict):
            score = float(res.get("score", res.get("reward", 0.0)))
            acc = float(res.get("acc", score > 0))
        else:
            score = float(res)
            acc = float(score > 0)
...
def get_task_components(task_type: str, config: Dict[str, Any] = None) -> tuple:
    ...
    if task_type == "countdown":
        ...
        # ttrl_math reward backend (same one used by on_policy_distillation.sh).
        prompt_processor = create_math_prompt_processor(
            system_message=system_message if not is_countdown_template else None,
            user_template=user_template if is_math_template else None,
