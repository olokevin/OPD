"""Hydra entry point for es_token (per-token weight-perturbation ES) training.
Mirrors main_np.py."""
import os
import socket
import tempfile
...
@hydra.main(config_path="config", config_name="es_token_trainer",
            version_base=None)
def main(config):
...
def run_es_token(config) -> None:
    from pprint import pprint
    print(f"es_token Training - hostname: {socket.gethostname()}, "
          f"PID: {os.getpid()}")
    ...
    if task_type in ["countdown", "gsm8k", "math", "math500", "olympiadbench",
                     "uspto50k", "common_gen", "mbpp", "rocstories",
                     "opd_math"]:
        ...
        reward_fn = (load_extern_object(config.data.reward_fn_path,
                                        config.data.reward_fn_name)
                     if config.data.get("reward_fn_path") else None)
        prompt_processor = (load_extern_object(
            config.data.prompt_processor_path,
    ...
    trainer = RayESTokenTrainer(config=config, tokenizer=tokenizer,
                                reward_fn=reward_fn, val_reward_fn=reward_fn,
