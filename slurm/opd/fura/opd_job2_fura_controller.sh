...
export CKPT_PATH=${DATA_ROOT}/checkpoints/job2_fura_dapo_lr1e-5
export EXPERIMENT_NAME=opd_fura_dapo_lr1e-5
export WANDB_RUN_ID=opd_fura_dapo_lr1e-5
...
echo "=== job2(fura) controller start $(date) | ckpt=$CKPT | nodes=$ALLOC_NODES | lr=$LR ==="
while [ "$ATTEMPT" -lt "$MAX_ATTEMPTS" ]; do
  ATTEMPT=$((ATTEMPT+1))
  START_ITER=$(cur_iter); START_ITER=${START_ITER:-0}
  RUNLOG=${DATA_ROOT}/logs/job2_fura_attempt${ATTEMPT}_$(date +%Y%m%d_%H%M%S).log
...
echo "=== job2(fura) controller end $(date) | final_iter=$(cur_iter) ==="
