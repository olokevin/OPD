...
echo "=== job2(fura76) controller start $(date) | ckpt=$CKPT | nodes=$ALLOC_NODES | lr=$LR ==="
while [ "$ATTEMPT" -lt "$MAX_ATTEMPTS" ]; do
  ATTEMPT=$((ATTEMPT+1))
  START_ITER=$(cur_iter); START_ITER=${START_ITER:-0}
  RUNLOG=${DATA_ROOT}/logs/job2_fura76_attempt${ATTEMPT}_$(date +%Y%m%d_%H%M%S).log
...
echo "=== job2(fura76) controller end $(date) | final_iter=$(cur_iter) ==="
