...
#   nohup bash slurm/opd_job1_full_controller.sh > /pscratch/sd/$USER/opd/logs/job1_controller_$(date +%Y%m%d_%H%M%S).log 2>&1 &
set -u
...
export ENV_SCRIPT=opd/full/opd_2node_env.sh                 # full (PEFT none)
...
# Keep ONLY the latest checkpoint (see opd_2node_controller.sh for rationale).
prune_loop() {
  while true; do
