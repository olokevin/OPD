#!/bin/bash
# zo_opd_2node_env.sh — OPD DRIVER env for the 2-node (8x A100 80GB) run. Runs as
...
# wandb, vLLM, NCCL, caches) lives in zo_opd_2node_rayenv.sh — sourced here for
...
export CKPT_PATH=${CKPT_PATH:-${DATA_ROOT}/checkpoints/zo_opd_qwen4b_1p7b_2node}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-zo_opd_qwen4b_1p7b_2node}
mkdir -p "${CKPT_PATH}"
...
# RAY_EXTERNAL=1 + RAY_ADDRESS are exported by the caller (zo_opd_2node_inside.sh).
