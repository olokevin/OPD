#!/bin/bash
# grpo_fura_controller.sh — FURA (BlockTT) GRPO on Qwen2.5-7B, 2 interactive nodes
# (8 GPUs), auto-resume across the 4h interactive cap. Reuses the validated
# opd_2node_inside.sh (ENV_SCRIPT selects the GRPO fura driver). Logs to wandb
# project nersc_grpo_qwen2p5_7b (offline; pushed by wandb_sync_daemon.sh).
#
#   nohup bash slurm/grpo/fura/grpo_fura_controller.sh > /pscratch/sd/$USER/opd/logs/grpo_fura_controller_$(date +%Y%m%d_%H%M%S).log 2>&1 &
set -u
...
export ENV_SCRIPT=grpo/fura/grpo_2node_env_fura.sh
export WANDB_PROJECT=nersc_grpo_qwen2p5_7b
export CKPT_PATH=${CKPT_PATH:-${DATA_ROOT}/checkpoints/grpo_fura_qwen2p5_7b_math}
export EXPERIMENT_NAME=grpo_fura_qwen2p5_7b_math
export WANDB_RUN_ID=grpo_fura_qwen2p5_7b_math
...
echo "=== grpo_fura controller start $(date) | ckpt=$CKPT | nodes=$ALLOC_NODES | lr=$LR ==="
while [ "$ATTEMPT" -lt "$MAX_ATTEMPTS" ]; do
  ATTEMPT=$((ATTEMPT+1))
  START_ITER=$(cur_iter); START_ITER=${START_ITER:-0}
  RUNLOG=${DATA_ROOT}/logs/grpo_fura_attempt${ATTEMPT}_$(date +%Y%m%d_%H%M%S).log
...
echo "=== grpo_fura controller end $(date) | final_iter=$(cur_iter) ==="
